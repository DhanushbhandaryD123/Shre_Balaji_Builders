  ...
  return (
    <section className="relative z-20 -mt-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <SectionReveal yOffset={25}>
        <div className="bg-white rounded-2xl shadow-xl border border-navy/5 p-6 sm:p-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 divide-y sm:divide-y-0 lg:divide-x divide-navy/5">
          {highlights.map((item, idx) => {
            const Icon = item.icon
            return (
              <div
                key={idx}
                className={`flex items-start gap-4 ${idx > 0 ? 'pt-4 sm:pt-0 lg:pl-6' : ''}`}
              >
